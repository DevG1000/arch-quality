...
> **版本绑定**
> - 对应《数值算法正确性与精度保障评估指南》**1.7 版**
> - 对应实现：`arch_metrics_numerical_accuracy.py`
> - 实现版本：**1.5 对齐**（`GUIDE_VERSION = "1.7"`, `SKILL_VERSION = "1.5"`）
> - 如需升级指南版本，需同步更新本 skill 和内省实现代码

## 引用文档

| 文档 | 版本 | 角色 |
|:-----|:----:|:-----|
| 《数值算法正确性与精度保障评估指南》 | 1.5 | 领域知识：维度定义、规则说明、评级标准 |
...
**代码坏味道：** "数值 TODO 堆积"——历史累积的数值技术债未清理，如已知精度问题、硬编码的容差、数值不稳定标记。

**检测方法：**
- 在源码中搜索 `FIXME`、`TODO`、`HACK`、`numerical`、`精度` 等关键词关联数值算法的注释
- 统计数值债务注释占总代码注释的比例
- 检测是否有已知精度问题的跟踪记录
...
```
num_debt_ratio = 数值债务标记行数 / 总注释行数

if num_debt_ratio <= 0.05:
  score = 100
elif num_debt_ratio <= 0.15:
  score = 70
elif num_debt_ratio <= 0.30:
  score = 40
else:
  score = 0
```
...
**参考：**
- 《数值算法正确性与精度保障评估指南（1.5版）》— `docs/zh/数值算法正确性与精度保障评估指南.md`
